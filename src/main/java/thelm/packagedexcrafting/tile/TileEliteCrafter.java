...
import thelm.packagedexcrafting.client.gui.GuiEliteCrafter;
import thelm.packagedexcrafting.container.ContainerEliteCrafter;
import thelm.packagedexcrafting.integration.appeng.networking.HostHelperTileEliteCrafter;
import thelm.packagedexcrafting.inventory.InventoryEliteCrafter;
...
public class TileEliteCrafter extends TileBase implements ITickable, IPackageCraftingMachine, IGridHost, IActionHost {
	...
	public TileEliteCrafter() {
		setInventory(new InventoryEliteCrafter(this));
		setEnergyStorage(new EnergyStorage(this, energyCapacity));
		if(Loader.isModLoaded("appliedenergistics2")) {
			hostHelper = new HostHelperTileEliteCrafter(this);
		}
	}

	@Override
	protected String getLocalizedName() {
		return I18n.translateToLocal("tile.packagedexcrafting.elite_crafter.name");
	}
	...
	public boolean acceptPackage(IRecipeInfo recipeInfo, List<ItemStack> stacks, EnumFacing facing) {
		if(!isBusy() && recipeInfo.isValid() && recipeInfo instanceof IRecipeInfoTiered) {
			...
			if(recipe.getTier() == 3) {
				ItemStack slotStack = inventory.getStackInSlot(49);
	...
	public boolean isBusy() {
		return isWorking || !inventory.stacks.subList(0, 49).stream().allMatch(ItemStack::isEmpty);
	}
	...
	protected void finishProcess() {
		...
		if(inventory.getStackInSlot(49).isEmpty()) {
			inventory.setInventorySlotContents(49, currentRecipe.getOutput());
		}
		else {
			inventory.getStackInSlot(49).grow(currentRecipe.getOutput().getCount());
		}
		for(int i = 0; i < 49; ++i) {
	...
	protected void chargeEnergy() {
		ItemStack energyStack = inventory.getStackInSlot(50);
		if(energyStack.hasCapability(CapabilityEnergy.ENERGY, null)) {
			...
			if(energyStack.getCount() <= 0) {
				inventory.setInventorySlotContents(50, ItemStack.EMPTY);
			}
		}
	}
	...
	public int getComparatorSignal() {
		...
		if(!inventory.stacks.subList(0, 50).stream().allMatch(ItemStack::isEmpty)) {
			return 15;
		}
		return 0;
	}

	public HostHelperTileEliteCrafter hostHelper;
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		if(nbt.hasKey("Recipe")) {
			...
			if(recipe instanceof IRecipeInfoTiered && ((IRecipeInfoTiered)recipe).getTier() == 3) {
	...
	public GuiContainer getClientGuiElement(EntityPlayer player, Object... args) {
		return new GuiEliteCrafter(new ContainerEliteCrafter(player.inventory, this));
	}

	@Override
	public Container getServerGuiElement(EntityPlayer player, Object... args) {
		return new ContainerEliteCrafter(player.inventory, this);
	}
}
